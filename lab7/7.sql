...
BEGIN
    SELECT
        topic.name topic_name, category.name AS category_name, books.name AS book_name, price, publishing.name AS publishing_name
    FROM
        books
    JOIN publishing ON books.id_publishing = publishing.id
...
BEGIN
    SELECT DISTINCT
        b.name AS 1 _book_name, b2.name 2 _book_name
    FROM
        books b
    JOIN books b2 ON
...
BEGIN
    ...
    WHERE EXISTS
        ...
        FROM
            publishing
        WHERE
            publishing.name = publisher_name AND books.id_publishing = publishing.id
    ) ;
END //
DELIMITER
    ;
